  ...
  User.findOne({ walletAddress: wallet })
    .then((data) => {
      if (!data)
        res
          .status(404)
          .send({ message: "Not found User with wallet " + wallet });
      else res.send(data);
    })
    .catch((err) => {
      res
        .status(500)
        .send({ message: "Error retrieving User with wallet=" + wallet });
    });
};
...
exports.updateUser = (req, res) => {
  ...
  // walletAddress is not already used
  if (req.body.walletAddress !== "") {
    User.find({
      walletAddress: req.body.walletAddress,
    })
      .then((data) => {
        if (data.length > 0 && data[0]._id != id) {
          res.status(400).send({
            message: "Email or walletAddress is already used!",
          });
        } else {
          User.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
            .then((data) => {
